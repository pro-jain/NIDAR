...
Blocks: 0.3m x 0.3m
Directory ma badha images process kare che
"""
...
# Directory containing GeoTIFF images (WebODM output - RGB Images)
...
def single_image(image_path):
    """ Single image process, yellow zones return kare che """
    ...
    if img_rgb.dtype != np.uint8:
        # Jo 16-bit hoy to convert karvu pade
...
def detect_for_multiple_images():
    """ Directory ma badha images process kare che  """
    ...
    # Find all GeoTIFF images
    image_files = []
    for ext in IMAGE_EXTENSIONS:
        image_files.extend(Path(IMAGE_DIR).glob(f'*{ext}'))
    
    if not image_files:
        print(f"Error: {IMAGE_DIR} no image found.")
        print(f"Supported extensions: {IMAGE_EXTENSIONS}")
        return
    
    print(f"\nTotal {len(image_files)} total images found.")
    ...
    for img_path in image_files:
        ...
        points = single_image(str(img_path))
        
        if points:
            all_results[img_name] = points
            all_points.extend(points)
    
    # Save combined results
    output_data = {
        "total_images_processed": len(image_files),
        "total_yellow_blocks": len(all_points),
        "by_image": all_results,
        "all_coordinates": all_points
    }
    ...
    print(f"Images with yellow zones: {len(all_results)}")
    print(f"Total yellow blocks detected: {len(all_points)}")
    print(f"Results saved to: {OUTPUT_JSON}")

if __name__ == "__main__":
    detect_for_multiple_images()